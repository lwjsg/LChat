...
//#include "loadingdlg.h" //这里没有使用新的窗口了
    ...
    _state(ChatUIMode::ChatMode),_b_loading(false),_cur_chat_uid(0),_last_widget(nullptr)
{
    ...
    QPixmap pixmap(":/Icon/head_1.jpg");
...
void ChatDialog::set_search_edit()
{
    ...
    clearAction->setIcon(QIcon(":/Icon/close_transparent.png"));
    // 初始时不显示清除图标
    ...
    connect(ui->search_edit, &QLineEdit::textChanged, [clearAction](const QString &text) {
        if (!text.isEmpty()) {
            clearAction->setIcon(QIcon(":/Icon/close_search.png"));
        } else {
            clearAction->setIcon(QIcon(":/Icon/close_transparent.png")); // 文本为空时，切换回透明图标
        }
    ...
    connect(clearAction, &QAction::triggered, [this, clearAction]() {
        ...
        clearAction->setIcon(QIcon(":/Icon/close_transparent.png")); // 清除文本后，切换回透明图标
